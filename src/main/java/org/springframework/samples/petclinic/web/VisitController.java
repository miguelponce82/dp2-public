...
public class VisitController {
	...
	public Collection<String> populateVets() {
		Collection<Vet> v=this.vetService.findVets();
		Collection<String> result = new ArrayList<String>();
		for (Vet vet: v){
			String s= vet.getFirstName();
			result.add(s);
		}
		return result;
	}
	...
	public String processNewVisitForm( @Valid Visit visit, BindingResult result, ModelMap model) {
		if (result.hasErrors()) {
			...
			catch(AlreadyBookedVisitException ex){
				result.rejectValue("date", "alreadyBooked", "already exists");
				visit.getPet().removeVisit(visit);
				return "pets/createOrUpdateVisitForm";
			}
	...
	public String showAllVisits(@PathVariable int petId, Map<String, Object> model) {
		
		model.put("visits", this.petService.findVisitsByPetId(petId));
